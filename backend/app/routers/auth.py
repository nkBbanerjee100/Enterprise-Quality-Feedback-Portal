...
async def forgot_password(
    ...
    """
    Email a 6-digit OTP if this address belongs to an active, registered
    account. Always returns the same generic message either way — revealing
    "no account with that email" would let anyone enumerate valid emails,
    so success/failure looks identical from the outside.
    """
    ip = get_client_ip(request)
    generic_message = {"message": "If this email is registered, a verification code has been sent."}
    ...
    if row is None or not row.is_active or not row.is_registered:
        log_action(
            local_db, action=AuditActions.PASSWORD_RESET_REQUESTED,
            ip_address=ip, success=False,
            details={"email": payload.email, "reason": "no_such_account_or_inactive"},
        )
